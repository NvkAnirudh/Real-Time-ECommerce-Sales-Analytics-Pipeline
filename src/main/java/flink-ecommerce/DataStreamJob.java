...
public class DataStreamJob {
	...
	public static void main(String[] args) throws Exception {
		...
		// Consumption of data from Kafka topic.
		String topic = "sales_transactions";

		// Deserializes the JSON-encoded byte array (Kafka message) into a Java object.
		...
		// create transactions table in Postgres
		...
		// create sales per category table sink in Postgres
		transactionStream.addSink(JdbcSink.sink(
		...
		// create sales per day table sink in Postgres
		transactionStream.addSink(JdbcSink.sink(
		...
		// create sales per month table sink in Postgres
		transactionStream.addSink(JdbcSink.sink(
		...
		// Insert into transactions table in Postgres
		transactionStream.addSink(JdbcSink.sink(
		...
		// Insert into sales per day table in Postgres
		...
		// Insert into sales per category table in Postgres
		transactionStream.map(
                        transaction -> {
                            Date transactionDate = new Date(System.currentTimeMillis());
		...
		// Insert into sales per month table in Postgres
		transactionStream.map(
                        transaction -> {
                            Date transactionDate = new Date(System.currentTimeMillis());
		...
		transactionStream.sinkTo(
			new Elasticsearch7SinkBuilder<Transaction>()
				...
				.setEmitter((transaction, runtimeContext, requestIndexer) -> {
					...
					System.out.println("Sending to Elasticsearch: " + json);  // Debugging log
